<?php
// Panggil file auth.php (ini otomatis start session & load database)
require_once __DIR__ . "/functions/auth.php"; 

// Panggil Satpam
checkLogin(); 

// Karena sudah lolos checkLogin, pasti user_id ada. Langsung pakai.
...
if ($user_id > 0 && isset($pdo)) {
    try {
        ...
        // Ambil semua task
        $stmt = $pdo->prepare("
            SELECT t.id, t.title, t.description, t.priority, t.deadline, t.status, t.category_id, c.name AS category
            FROM tasks t
        ...
        $userCategories = []; // Siapkan array kosong
        try {
            $stmtCat = $pdo->prepare("SELECT id, name FROM categories WHERE user_id = ? ORDER BY name ASC");
            $stmtCat->execute([$user_id]);
            $userCategories = $stmtCat->fetchAll(PDO::FETCH_ASSOC);
        } catch (Exception $e) {
            // Silent error agar dashboard tetap jalan meski kategori gagal load
        }
                    ...
                    <div class="modal-body">
                        ...
                        <div class="mb-3">
                            ...
                            <small class="text-muted">
                                <a href="categories.php" style="text-decoration: none;">+ Create new category</a>
                            </small>
                        </div>
                        <div class="mb-3">
            ...
            <div class="modal-content">
                <form id="editTaskForm">
                    ...
                    <div class="modal-body">
                        ...
                        <div class="mb-3"><label>Title</label><input type="text" name="title" id="editTitle"
                                class="form-control" required></div>
                        <div class="mb-3"><label>Description</label><textarea name="description" id="editDesc"
                        ...
                        <div class="mb-3"><label>Deadline</label><input type="date" name="deadline" id="editDeadline"
                        ...
                        <td class="actions">
                            ...
                            <button class="action-btn edit-btn" onclick="openEdit(this.closest('tr'))" title="Edit">
                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                                    stroke="currentColor">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                        d="M15.232 5.232l3.536 3.536M9 11l6-6m2 2l-6 6m-2 2H6v-3" />
                                </svg>
                            </button>
